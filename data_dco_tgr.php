...
$count_query = "SELECT COUNT(*) AS total 
                FROM trans tr
                INNER JOIN barang_tgr b ON tr.id_barang_tgr = b.id
...
if ($total_results > 0) {
    ...
    if ($destination || $startDate || $endDate) {
        $query = "CALL GetFilteredDatatgr($starting_limit_number, $results_per_page, '$destination', '$startDate', '$endDate')";
    } else {
        $query = "CALL GetDCOtgr($starting_limit_number, $results_per_page)";
    }
            ...
            <?php if (isset($number_of_pages) && $number_of_pages >= 1): ?>
                <div class="pagination mt-4">
                    <?php for ($page = 1; $page <= $number_of_pages; $page++): ?>
                        <a href="data_dco_tgr.php?page=<?php echo $page; ?>"
