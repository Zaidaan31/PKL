...
$count_query = "SELECT COUNT(*) AS total 
                FROM trans tr
                INNER JOIN barang_bgr b ON tr.id_barang_bgr = b.id
...
if ($total_results > 0) {
    ...
    if ($destination || $startDate || $endDate) {
        $query = "CALL GetFilteredDatabgr($starting_limit_number, $results_per_page, '$destination', '$startDate', '$endDate')";
    } else {
        $query = "CALL GetDCObgr($starting_limit_number, $results_per_page)";
    }
            ...
            <?php if (isset($number_of_pages) && $number_of_pages >= 1): ?>
                <div class="pagination mt-4">
                    <?php for ($page = 1; $page <= $number_of_pages; $page++): ?>
                        <a href="data_dco_bgr.php?page=<?php echo $page; ?>"
